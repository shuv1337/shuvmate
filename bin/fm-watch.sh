...
# This is the backend target (tmux:ses:name or zellij:ses:tab:pane) the staleness
# scan captures through fm-mux.sh; see the Layer 1 loop below.
meta_target() {
...
EOF
  ...
  for meta in "$STATE"/*.meta; do
    ...
    captarget=$(meta_target "$meta")
    [ -n "$captarget" ] || captarget="tmux:$w"
    tail40=$("$MUX" capture "$captarget" 40 2>/dev/null) || continue
