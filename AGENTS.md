...
config/crew-model    claude model override for all firstmate-launched claude agents (ship/scout crewmates and secondmate supervisors); LOCAL, gitignored; a single model token (opus, sonnet, haiku, fable); absent or "default" = opus; a per-spawn FM_CREW_MODEL env var overrides it for one dispatch
...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: mux=, target=, window=, worktree=, project=, harness=, kind=, mode=, yolo=; kind=secondmate also records home= and projects= (fm-pr-check appends pr= and verified pr_head= when available; fm-x-link appends x_request= and x_request_ts= for an X-mention-originated task, section 14); missing mux= means legacy tmux
  <id>.check.sh      optional slow poll you write per task (e.g. merged-PR check)
  captain-view.html  generated HTML operator view of captain asks + backlog; regenerated by fm-captain-asks.sh mutations, refreshed by hand with bin/fm-asks-html.sh (section 9.1)
...
If the captain names a different claude model, write it to `config/crew-model` (local, gitignored): a single model token, absent or `default` meaning the `opus` baseline; that model is passed as `--model` to every firstmate-launched claude agent, including ship/scout crewmates and secondmate supervisors.
...
Orthogonal to harness, every firstmate-launched claude agent - ship/scout crewmates and secondmate supervisors - launches on a resolved model rather than the claude CLI's own default (Fable, the priciest tier) for routine coding, review, and supervision work.
...
`bin/fm-spawn.sh` injects that model as `--model` into the shared claude launch template only; codex, opencode, and pi carry their own model mechanisms and are untouched. A separate secondmate model could be added later if the fleet ever needs that split.
For a one-off per-spawn override ("run this one on sonnet"), set `FM_CREW_MODEL` in the spawn's environment (`FM_CREW_MODEL=sonnet bin/fm-spawn.sh ...`); it wins over `config/crew-model` for that dispatch only.
...
All truth lives in the configured multiplexer, state files, data/backlog.md, data/secondmates.md, persistent secondmate homes, and treehouse; your conversation memory is a cache.
...
For `kind=secondmate`, the script creates the same kind of surface but starts directly in the persistent home.
...
Every mutating verb (`add`, `resolve`, `sync-from-state`) also regenerates `state/captain-view.html`, a self-contained HTML rendering of the open asks and `data/backlog.md` the captain can open in a browser; the regeneration is best-effort and a render failure never fails the mutation.
Backlog changes are hand-edited, so after editing `data/backlog.md` refresh the view with `bin/fm-asks-html.sh` (it prints the output path; `bin/fm-asks-html.sh path` prints it without rendering).
