...
When either role launches through Claude, `config/crew-model` (or one-spawn `FM_CREW_MODEL`) pins the model token, defaulting to `opus`, while non-Claude adapters keep their own model handling.
...
All state lives in the configured multiplexer, no-mistakes run records, status event logs, local markdown under `data/`, `data/secondmates.md`, and persistent secondmate homes.
